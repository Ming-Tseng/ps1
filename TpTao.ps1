...
'Q_vil_adj_traffic',     E:\everyday-cron\Q_importErrorLog\Q_vil_adj_traffic.txt
'Q_vil_adm_remedy_log',  E:\everyday-cron\Q_importErrorLog\Q_vil_adm_remedy_log.txt
'Q_vil_car_susp_his',    E:\everyday-cron\Q_importErrorLog\Q_vil_car_susp_his.txt
'Q_vil_div_detail',      E:\everyday-cron\Q_importErrorLog\Q_vil_div_detail.txt
'Q_vil_migrate',         E:\everyday-cron\Q_importErrorLog\Q_vil_migrate.txt

'Q_vil_motor_susp_his',  E:\everyday-cron\Q_importErrorLog\Q_vil_motor_susp_his.txt
'Q_vil_phone_xact',      E:\everyday-cron\Q_importErrorLog\Q_vil_phone_xact.txt
'Q_vil_plate_susp_his',  E:\everyday-cron\Q_importErrorLog\Q_vil_plate_susp_his.txt
'Q_vil_query_log',       E:\everyday-cron\Q_importErrorLog\Q_vil_query_log.txt
'Q_vil_receipt_payment', E:\everyday-cron\Q_importErrorLog\Q_vil_receipt_payment.txt

'Q_vil_traffic_rule'     E:\everyday-cron\Q_importErrorLog\Q_vil_traffic_rule.txt
...
$dbname ="TAODW_CSQ"
$dr="@#"
$ilogfile='E:\everyday-cron\everyQuarterly.txt'  #  ii $ilogfile  ,   ri  $ilogfile
...
(get-date -Format yyyyMMdd-hhmmss).tostring( )    +  '     Line  57    start    '| out-file   $ilogfile -Append 
...
#}--
        ...
        'm3_vil_div_his' {  $tableName="vil_div_his"   }}
         ...
         $fullpathH=$f1+'\'+$tableName+'.H'  #ii D:\olapdb\

    if (  (test-path $fullpathD ) -and ( test-path $fullpathH  )  ) {          $fi='1'   }
    if ( $fi -eq '1' ){  
#          cp   "D:\olapdb\$tableName.D"  "$f2\$tableName.D"  -Force
          cp   $fullpathD   "$f2\$tableName.D"  -Force  
          cp  $fullpathH   "$f2\$tableName.H"  -Force  
      }
}

$t2=get-date;
(get-date -Format yyyyMMdd-hhmmss).tostring( )    +  '     Line  155   copy   F1 to F2    ' + ($t2-$t1).TotalMinutes| out-file   $ilogfile -Append 
...
if ( $fi -eq '1' ){    cp  D:\olapdb\*.*  "$f3\$dtcopy(完整)" -Force    }
$t2=get-date;
(get-date -Format yyyyMMdd-hhmmss).tostring( )    +  '     Line  196   copy   F1 to F3    ' + ($t2-$t1).TotalMinutes| out-file   $ilogfile -Append 
...
$t2=get-date;#($t2-$t1).TotalMinutes #--41 sec
(get-date -Format yyyyMMdd-hhmmss).tostring( )    +  '     Line  308   import  main  use Powershell    ' + ($t2-$t1).TotalMinutes| out-file   $ilogfile -Append 

#-----------------------------------
#     import  main  use dtsx
#-----------------------------------
$t1=get-date;#($t2-$t1).TotalMinutes #--41 sec
...
$t2=get-date;#($t2-$t1).TotalMinutes #--41 sec
(get-date -Format yyyyMMdd-hhmmss).tostring( )    +  '     Line  360   import  main  use dtsx   ' + ($t2-$t1).TotalMinutes| out-file   $ilogfile -Append 
...
$t2=get-date;#($t2-$t1).TotalMinutes #--41 sec
(get-date -Format yyyyMMdd-hhmmss).tostring( )    +  '     Line  396    compart  file count & Table count  ' + ($t2-$t1).TotalMinutes| out-file   $ilogfile -Append 
...
#checkLog  2016-05-30
$t2=get-date;#($t2-$t1).TotalMinutes #--41 sec
(get-date -Format yyyyMMdd-hhmmss).tostring( )    +  '     Line  406   delete F1 & F2  ' + ($t2-$t1).TotalMinutes| out-file   $ilogfile -Append 
