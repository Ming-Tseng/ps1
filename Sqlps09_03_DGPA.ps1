                C:\Users\administrator.CSD\SkyDrive\download\ps1\Sqlps09_03_DGPA.ps1\\172.16.220.29\c$\Users\administrator.CSD\SkyDrive\download\ps1\Sqlps09_03_DGPA.ps1<#  Sqlps09_03_DGPA auther : ming_tseng    a0921887912@gmail.com createData : Nov.01.2014 for DGPA history :  object :    Invoke-Sqlcmd -Query 'select @@SERVERNAME' -ServerInstance $para_svrinstance -Database $para_database #-QueryTimeout Phase II : from  66 database to  NHRDB#>#----------------------------------------------------------------------------
#    main  Nov.01.2014
#----------------------------------------------------------------------------
...
{  #p.114 for filter
$tname=$GetArt.tablename;#$tname  -- 正在加入交易式發行項
$filtername=$pdbname.Substring(0,10);#$filtername
$tID=$tname.Substring(5,2)+'IDNO'

$tsql5=@"
use [$pdbname]
exec sp_addarticle @publication = N'$publicationName', @article = N'$tname'
, @source_owner = N'dbo', @source_object = N'$tname'
, @type = N'logbased', @description = N'', @creation_script = N'', @pre_creation_cmd = N'delete'
, @schema_option = 0x000000000803509F, @identityrangemanagementoption = N'none'
, @destination_table = N'$tname'
, @destination_owner = N'dbo', @status = 24, @vertical_partition = N'false'
, @ins_cmd = N'CALL [sp_MSins_dbo$tname]'
, @del_cmd = N'CALL [sp_MSdel_dbo$tname]'
, @upd_cmd = N'SCALL [sp_MSupd_db$tname]'
, @filter_clause = N'B$tID in (select B02IDNO from restable Where B02SEDORG=''$filtername'')'


-- 正在加入發行項篩選
exec sp_articlefilter @publication = N'PhaseII', @article = N'CPABT02M'
, @filter_name = N'FLTR_$tname-1__147'
, @filter_clause = N'B$tID in (select B02IDNO from restable Where B02SEDORG=''379000000A'')'
, @force_invalidate_snapshot = 1
, @force_reinit_subscription = 1

-- 正在加入發行項同步處理物件
exec sp_articleview @publication = N'PhaseII', @article = N'CPABT02M'
, @view_name = N'SYNC_CPABT02M_1__147'
